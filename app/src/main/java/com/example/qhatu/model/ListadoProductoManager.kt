...
class ListadoProductoManager {
    fun obtenerListaProducto(id_categoria_clickeada : String, databaseReference : FirebaseFirestore, block:(lista:ArrayList<Producto>) -> Unit) {

        var id : Long = 1
        val lista = ArrayList<Producto>()
        val product_ref = databaseReference.collection("users")
        product_ref.get().addOnSuccessListener {
            for (doc in it){
                val subcollection = doc.reference.collection("purchase-list")
                subcollection.get().addOnSuccessListener {
                    for(doc_2 in it){
                        val id_categoria = (doc_2["product-category"] as DocumentReference).id
                        if(id_categoria == id_categoria_clickeada){
                            val prod = doc_2["quantity"].toString() + " " + doc_2["product-name"].toString()  +
